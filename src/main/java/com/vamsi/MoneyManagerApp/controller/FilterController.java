...
public class FilterController {
    ...
    public ResponseEntity<?> filterTransaction(@RequestBody FilterDTO filterDTO) {
        ...
        if ("income".equalsIgnoreCase(filterDTO.getType())) {
            return new ResponseEntity<>(incomeService.filterIncomes(startDate, endDate, keyword, sort), HttpStatus.FOUND);
        } else if ("expense".equalsIgnoreCase(filterDTO.getType())) {
            return new ResponseEntity<>(expenseService.filterExpenses(startDate, endDate, keyword, sort), HttpStatus.FOUND);
        } else {
